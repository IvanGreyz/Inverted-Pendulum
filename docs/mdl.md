...
$$Ma = \sum_{n=1}^{\infty}{F} = \vec{P}_M + \vec{N} + \vec{F} + \vec{U}$$ Where $a$ is the acceleration of cart. Decomposite $a_M$ into $x$, $y$ direction:
...
$$ma_p = \sum_{n=1}^{\infty}{F} = \vec{P}_m + \vec{F}$$ The $a_p$ is the acceleration of pole from cart position. Decomposite $a_p$ to get:
...
$$\sum{\tau} = \frac{d\vec{H}}{dt} + \vec{v} \times \vec{R}_{CP}$$ Where $\frac{d\vec{H}}{dt}$ is the Time rate change of Angular momentum of pole rod, $\vec{v} \times \vec{R}_{CP}$ is the Time rate change of Angular momentum of pole rod causing by cart motion, $\vec{v}$ is the Velocity of cart and $\vec{R}_{CP}$ is the Displacement vector from cart to pole.
